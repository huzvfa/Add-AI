...
def get_anthropic_client():
    key = os.environ.get("ANTHROPIC_API_KEY") or st.session_state.get("anthropic_key", "")
...
def enhance_prompt_with_claude(client, prompt, mode):
    """Use Claude to enhance the user's prompt for better UGC content generation."""
    system = """You are a UGC (User Generated Content) creative director. 
    Your job is to transform basic prompts into highly detailed, cinematic generation prompts 
    that produce viral-quality content. Think: authentic lifestyle content, relatable scenarios, 
    product showcases, before/after transformations, social proof moments, trending aesthetics.
    Keep enhanced prompts under 500 characters for image models.
    Return ONLY the enhanced prompt, nothing else."""
    
    mode_context = {
        "text-image": "optimized for FLUX/Stable Diffusion image generation. Add style, lighting, camera angle, mood descriptors.",
        "image-image": "for image-to-image transformation. Focus on style transfer, mood, artistic direction.",
        "text-video": "for video generation. Add motion, scene progression, cinematic details, pacing.",
        "image-video": "for animating a still image into video. Add motion direction, camera movement, animation style."
    }
    ...
    try:
        response = client.messages.create(
            ...
            messages=[{
                "role": "user",
                "content": f"Mode: {mode_context.get(mode, '')}\n\nOriginal prompt: {prompt}\n\nEnhance this for UGC content creation:"
            }]
        )
...
def render():
            ...
            if not t2i_prompt.strip():
                ...
                with st.status("🎨 Creating your UGC image...", expanded=True) as status:
                    ...
                    if t2i_enhance and client:
                        ...
                        final_prompt = enhance_prompt_with_claude(client, 
        ...
        if st.button("🔄 Transform Image", key="i2i_gen", use_container_width=True):
            if not i2i_upload:
                ...
                with st.status("🔄 Transforming your image...", expanded=True) as status:
                    ...
                    if client:
                        final_prompt = enhance_prompt_with_claude(client, final_prompt, "image-image")
            ...
            if not t2v_prompt.strip():
                ...
                with st.status("🎬 Creating your UGC video...", expanded=True) as status:
                    ...
                    if client:
                        ...
                        final_prompt = enhance_prompt_with_claude(client, final_prompt, "text-video")
    ...
    with st.sidebar:
        ...
        with st.expander("Configure API Keys", expanded=not get_replicate_token()):
            ...
            ant_key = st.text_input("Anthropic (for AI enhance)", type="password",
                                     placeholder="sk-ant-...",
                                     value=st.session_state.get("anthropic_key", ""),
                                     key="ugc_ant_key")
            if ant_key: st.session_state.anthropic_key = ant_key
            ...
            <div style="font-size:0.75rem;color:#6b7280;margin-top:0.75rem;">
              ...
              • <a href="https://console.anthropic.com" target="_blank" style="color:#00f5d4;">console.anthropic.com</a>
            </div>
            """, unsafe_allow_html=True)
